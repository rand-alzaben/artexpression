    ...
    <div className='max-w-lg flex flex-col gap-2 text-sm'>
...

      {
        isEdit
          ...
          : <p className='font-medium text-3xl text-primary mt-4' >{userData.name}</p>
      }
      ...
      <div>
        <p className='text-neutral-500 underline mt-3'>ADDITIONAL INFORMATION</p>
        <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-neutral-700'>
          <p className='font-medium'>Gender:</p>
          {
            isEdit
              ...
              : <p className='text-pink-400'>{userData.gender}</p>
          }
          <p className='font-medium' >Birthday:</p>
          {
            isEdit
              ? <input className='max-w-28 bg-gray-100' type="date" onChange={(e) => setUserData(prev => ({ ...prev, dob: e.target.value }))} value={userData.dob} />
              : <p className='text-pink-400'>{userData.dob}</p>
          }
        </div>
      </div>
